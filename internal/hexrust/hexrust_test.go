...
func TestTwoCellsThatEachDeclareABuildScriptShareOneFnMainCallingBothInCellNameOrder(t *testing.T) {
	...
	write := writeUnder(t, root)
	write(filepath.Join("src", "authz_client", hexrust.CellConfigFile), "name: songe-hello\nkind: grpc\n")
	write(filepath.Join("src", "authz_client", cellmanifest.FileName), string(body))
...
drivers:
  rest: { enabled: true }
  grpc: { enabled: true }
`, "grpc", "rest", "authz_client")

	build := files["zz_generated_build.rs"]

	want := `include!("src/authz_client/zz_generated_build.rs");
include!("src/grpc/zz_generated_build.rs");
...
`
	...
	if !strings.HasSuffix(build, want) {
		t.Fatalf("the crate root build script\n got %s\nwant it to end with\n%s", build, want)
	}
}
