...
func TestA401OnAnOperationWithoutXAuthArmsTheControllerAuthenticationErrorInsteadOfTheVerifier(t *testing.T) {
	vectors := `{"cases": [{"case": "a", "operation": "getGreeting", "input": {"id": "g1"}, "expectedStatus": 401, "expectedErrorSubstring": "stranger"}]}`

	files, err := vectorsrust.Generate([]byte(guardedSpec), []byte(vectors), vectorsrust.Options{Service: "songe-hello"})
	if err != nil {
		t.Fatalf("generating: %v", err)
	}

	want := `Err(GreetingControllerError::Authentication { subject: "stranger".to_string(), reason: "generated by vectors-rust".to_string() })`
	if !strings.Contains(files[0].Content, want) {
		t.Fatalf("the emitted test lacks %q:\n%s", want, files[0].Content)
	}

	if strings.Contains(files[0].Content, "greeting_controller.expect_get_greeting().never();") {
		t.Fatal("a 401 answered by the controller must not arm the controller with never")
	}
}
