...
func Run(_ context.Context, input mcptypes.RunInput, spec *Spec) (*forge.TestReport, error) {
	...
	findings, err := forgelint.Check(forgelint.Options{RootDir: rootDir})
	...
	if err != nil {
		...
		report.ErrorMessage = fmt.Sprintf("checking forge.yaml under %q: %v", rootDir, err)
		return report, nil
	}
	...
	if len(findings) > 0 {
		...
		fmt.Fprintf(&details, "Found %d forge.yaml violation(s)\n\n", len(findings))
