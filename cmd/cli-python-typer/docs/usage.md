...
A forge-dev generator that fills the cli x python cell with a typer
dispatcher. Same behavior contract as every cli cell:

```yaml
kind: cli
language: python
generator: forge://github.com/alexandremahdhaoui/forge-dev-codegen/cmd/cli-python-typer
surface:
  commands:
    - name: greet
