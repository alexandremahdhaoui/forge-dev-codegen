...
A forge-dev generator that fills the cli x typescript cell with a commander
dispatcher. Same behavior contract as every cli cell:

```yaml
kind: cli
language: typescript
generator: forge://github.com/alexandremahdhaoui/forge-dev-codegen/cmd/cli-typescript-commander
layout:
  commands:
    - name: greet
