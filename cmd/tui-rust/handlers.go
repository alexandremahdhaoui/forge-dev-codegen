...
func NewHandlers() Handlers {
	return Handlers{
		Generate: func(_ context.Context, input GenerateInput) (*GenerateOutput, error) {
			if input.Kind != "tui" {
				return nil, fmt.Errorf("emitting %q: tui-rust fills the tui cell only", input.Kind)
			}

			if input.Language != "" && input.Language != "rust" {
				return nil, fmt.Errorf("emitting for %q: tui-rust generates rust only", input.Language)
			}

			files, err := tuirust.Generate([]byte(input.WiringSpec), tuirust.Options{
				Service: input.Name,
				Cell:    layoutString(input.Layout, "cell"),
			})
			if err != nil {
				return nil, fmt.Errorf("emitting the tui cell of %q: %w", input.Name, err)
			}
