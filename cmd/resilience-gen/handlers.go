...
func NewHandlers() Handlers {
	return Handlers{
		Generate: func(_ context.Context, input GenerateInput) (*GenerateOutput, error) {
			if input.Kind != "resilience" {
				return nil, fmt.Errorf("emitting %q: resilience-gen fills the resilience cell only", input.Kind)
			}
			...
			files, err := concerns.EmitResilience(lang)
			if err != nil {
				return nil, fmt.Errorf("emitting resilience for %q: %w", input.Name, err)
			}
