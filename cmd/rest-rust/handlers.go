...
func NewHandlers() Handlers {
	return Handlers{
		Generate: func(_ context.Context, input GenerateInput) (*GenerateOutput, error) {
			if input.Kind != "rest" {
				return nil, fmt.Errorf("emitting %q: rest-rust fills the rest cell only", input.Kind)
			}

			if input.Language != "" && input.Language != "rust" {
				return nil, fmt.Errorf("emitting for %q: rest-rust generates rust only", input.Language)
			}
